...
//
    ...
    //  For more info.
    //
            _objSections[__sect('header')]   = Section();
...
///////////////////////////////////////////////////////////
//
        const   __set_header_mode       = tokens =>
        {
            
            if (tokens.length != 4)
                return messenger.file_error(tokens, `The mode instruction requires exactly one parameter`);

            if (typeof tokens[3] === 'number')
                tokens[3] = tokens[3].toString();

            if (tokens[3] !== '1' && tokens[3] !== '2' && tokens[3] !== '4')
                return messenger.file_error(tokens, `'${tokens[3]}' is not a valid parameter for the mode instruction`);
    
            const   __header_index = _objSections[0].label.indexOf(tokens[3]);

            if (__header_index < 0)
            {
                _objSections[0].label.push('mode');
                _objSections[0].data.push(tokens[3]);
            }
            else
                _objSections[0].data[__header_index] = tokens[3];
...
///////////////////////////////////////////////////////////
//
        const   __set_header_model      = tokens =>
        {
            
            let     __s16_model;

            if (tokens.length != 4)
                return messenger.file_error(tokens, `The model instruction requires exactly one parameter`);

            if (tokens[3] === 'flat')
                __s16_model = 1;
            else if (tokens[3] === 'dual')
                __s16_model = 2;
            else if (tokens[3] === 'multi')
                __s16_model = 3;
            else if (tokens[3] === 'split')
                __s16_model = 4;
            else
                return messenger.file_error(tokens, `'${tokens[3]}' is not a valid parameter for the model instruction`);
    
            const   __header_index = _objSections[0].objSection.label.indexOf(tokens[2]);

            if (__header_index < 0)
            {
                _objSections[0].label.push('model');
                _objSections[0].data.push(__s16_model);
            }
            else
                _objSections[0].data[__header_index] = __s16_model;
...
///////////////////////////////////////////////////////////
//
        const   __set_offset_value      = tokens =>
        {

            if (tokens.length !== 4)
                return messenger.file_error(tokens, `The ${tokens[2]} instruction requires exactly one parameter`);

                if (typeof tokens[3] === 'string')
                {
                    if (! /^[0-9]+$/.test(tokens[3]))
                        return messenger.file_error(tokens, `'${tokens[3]}' is not a valid parameter for the '${tokens[2]} instruction`);
                }
                if (tokens[2] < 0 || tokens[2] > 0xFFFFFFFF)
                    return messenger.file_error(tokens, `'${tokens[3]}' is not a valid parameter for the '${tokens[2]} instruction`);

                const   __header_index  = _objSections[0].label.indexOf(tokens[2]);

                if (__header_index < 0)
                {
                    _objSections[0].label.push(tokens[2]);
                    _objSections[0].data.push(tokens[3]);
                }
                else
                    _objSections[0].data[__header_index] = tokens[3];
...
//
        const   __process_header_tokens = tokens =>
        {
            ...
            if (typeof _objSections[0].label === 'undefined')
            {
                _objSections[0].label = [];
                _objSections[0].data = [];
            }

            if (tokens[2] === 'mode')
                return __set_header_mode(tokens);

            if (tokens[2] === 'model')
                return __set_header_model(tokens);

            if (tokens[2] === 'segments')
                return __set_offset_value(tokens);

            if (tokens[2] === 'maxaddr')
                return __set_offset_value(tokens);

            messenger.file_error(`Unknown header instruction '${tokens[2]}`);
...
//
    ...
    //
            ...
            if (typeof _size === 'string')
                messenger.file_error(tokens, _size);
